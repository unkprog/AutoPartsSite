...
-- из таблицы [PricesFilesCalcTypes] - берем условия, на основе которых производится группировка данных (минимальная цена, максимальное наличие - см. выше таблицу [PricesFilesCalcTypes])
     ...
     , [a].[PriceCurrencyID]
	 , [SeparatorSymbol.ID] = [pff].[SeparatorSymbolID], [SeparatorSymbol.Code] = [ss].[Code], [SeparatorSymbol.Symbol] = [ss].[Symbol]
	 , [FractionalSymbol.ID] = [pff].[FractionalSymbolID], [FractionalSymbol.Code] = [fs].[Code], [FractionalSymbol.Symbol] = [fs].[Symbol]
